...
function start() {
  ...
  console.log(chalk.bold(`${EOL}Давай начнем, ${chalk.bgAnsi256(random)(`${userName}`)}!`));
  ...
  if (themes[index] === elbrusQuestions[0].theme) {
    for (let i = 0; i < elbrusQuestions.length; i++) {
      console.log(chalk.bgMagenta`У тебя ${userPoints} баллов`);
    ...
    for (let i = 0; i < newYearQuestions.length; i++) {
      console.log(chalk.bgCyan`У тебя ${userPoints} баллов`);
